...
import 'package:jaspr_notifier/src/context/reactive_inherited_context.dart';
...
class ReactiveContextEnhanced {
  ...
  static T getReactiveState<T>(
    ...
    if (inheritedWidget != null) {
      assert(() {
        log('[ReactiveContext] Using InheritedWidget strategy for $T');
        return true;
      }());
    ...
    assert(() {
      log('[ReactiveContext] Using markNeedsBuild strategy for $T');
      return true;
    }());

    final element = context as Element;

    // Register for type-specific rebuilds
    // TODO: Jaspr might need a different approach for post-frame callbacks
    // For now, register immediately
  ...
  static void _registerForTypeSpecificRebuilds<T>(
    ...
    if (!_globalListenersSetup.containsKey(notifier)) {
      notifier.listen((newValue) {
        ...
        // Note: In Jaspr, Element doesn't have a mounted getter
        // markNeedsBuild() safely handles inactive elements
        _markNeedsBuildElements[notifier]?.removeWhere((e) {
          try {
            e.markNeedsBuild();
            return false; // Keep element if markNeedsBuild succeeded
          } catch (_) {
            return true; // Remove element if it's disposed
          }
        });
        ...
        if (elementsForNotifier != null) {
          ...
          for (final elem in elementsForNotifier) {
            try {
              ...
              // Ignore errors for disposed elements
            }
          }
        }
      });
...
///
/// This is a public API component that developers can use
class ReactiveContextBuilder extends StatelessComponent {
  ...
  Component build(BuildContext context) {
    ...
    // Create InheritedComponents for all specified notifiers
    for (final notifier in forceInheritedFor.reversed) {
      current = _createInheritedComponent(notifier, current);
    }
