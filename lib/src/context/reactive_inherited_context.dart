...
/// Internal InheritedComponent that provides reactive state through ReactiveNotifier
/// This component is created dynamically and injected into the component tree automatically
/// ```when context<T>()``` or extensions like context.lang are used
class ReactiveInheritedContext<T> extends InheritedComponent {
  /// The type identifier for this reactive context
  final Type contextType;

  /// The notifier that provides the reactive state
  ...
  /// Find an existing ReactiveInheritedContext of type T in the component tree
  static ReactiveInheritedContext<T>? maybeOf<T>(BuildContext context) {
    return context
        .dependOnInheritedComponentOfExactType<ReactiveInheritedContext<T>>();
  }
  ...
  @override
  bool updateShouldNotify(ReactiveInheritedContext<T> oldComponent) {
...
''', level: 50);
    ...
    // It will be stored when properly injected into component tree
    final context = ReactiveInheritedContext<T>(
      notifier: notifier,
      contextType: contextType,
      child: span([]), // Placeholder child
    );
